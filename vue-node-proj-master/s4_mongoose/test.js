...
var db = mongoose.connect('mongodb://127.0.0.1:27017/test'); // todo  test为数据库的名称
...
var testModel = db.model('test1', testSchema); // todo test1为集合名称，即数据库表名称；集合的结构对象
// Document文档（关联数组式的对象） < Collection集合 < 数据库

// todo step1: 插入数据
// todo **** 插入数据，一次执行完后注释掉 start ****
// 插入保存一段数据
testModel.create([
  {name: "test1", age: 8},
  {name: "test2", age: 18},
...
// todo step4: 数据更新 (先修改，后查询)
// 基本使用：model.update(查询条件,更新对象,callback);
var conditions = {name: 'test1'}; // 查询条件
var update = {$set: {age: 11 }};  // 待修改对象
testModel.update(conditions, update, function(error){ // todo 1. 先修改
  if(error) {
    console.log(error);
  } else {
    console.log('Update success!');
    testModel.find({name: 'test1'}, {name:1, age:1, _id:0}, function(err, docs){  // todo 2.后查询
      if (err) {
        console.log('查询出错：' + err);
      } else {
...
// todo step5: 数据删除
// todo 基本使用：model.remove(查询条件,callback); (先删除，后查询)
var conditions = {name: 'test2'};
testModel.remove(conditions, function(error){ // todo 1. 先删除
  if(error) {
    console.log(error);
  } else {
    console.log('Delete success!');
    testModel.find({name: 'test2'}, {name:1, age:1, _id:0}, function(err, docs){  // todo 2.后查询
      if (err) {
        console.log('查询出错：' + err);
      } else {
